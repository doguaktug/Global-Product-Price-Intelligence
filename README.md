...
2. **User selects preference weights** (optional sliders; country/currency: configured default → manual override, recorded in `origin`; geolocation is a documented proposal, not code)  
   `domain/models.py` → `UserPreferences` · `config.py` → `DEFAULT_DESTINATION_COUNTRY`, `DEFAULT_REFERENCE_CURRENCY` · wired through `start_search` / `StartSearchRequest.preferences` · `orchestrator/search.py` → `SearchOrchestrator._resolve_preferences` · *UI sliders: not built yet*
