...
The value tests decide **which alternatives are shown**. A same-family variant is badged an upgrade if some spec gains at least 25% while the landed cost rises at most 10%. It is badged a downgrade if it saves at least 15% while no spec falls more than 50% and it still meets the confirmed minimum. It is badged similar if specs, landed price, and score all stay close (within about 10% / 10% / 10% of the top pick). A different product is badged a rival if it shares the category, overlaps at least 60% of the core specs, scores above 85% of the best-for-you score, and stays within 15% on landed price.
...
A near-offer that passes no test is omitted — not listed "for comparison." The panel is for recommendations the system can stand behind.

The system keeps at most three alternatives and fills those slots with one of each badge where it can, before topping up with the highest-scoring remaining badged candidates — three cheaper-but-smaller variants would tell the user one thing three times. Each alternative also carries its landed cost **minus the top pick's**, so a negative figure means it is cheaper; the panel exists to answer what switching would cost, and a reader should not have to subtract two totals to find out. If no near-offer clears a value test, the panel is empty.
...
The Decision Page then shows those cards. It also shows the original price, the FX rate and its publication date, the landed-cost add-ons, and the time each offer was collected. The product name on each card is the retailer link and opens the listing in a new tab. Whether an offer can actually be bought was settled during the search — an out-of-stock listing never entered the ranking, and one with unknown stock was scored down for it — so there is no second check when the user clicks through.
